...
Schrodinger1D::Schrodinger1D(const Axis *F, Type timeStep): PDELinear1DVirtual(Frame(F), 0, 1)
{
...
{
	LS->SORMethod(m_Space->getColumnAtTime(0), m_Space->getColumnAtTime(1));
	m_Space->switchGrid();
}
...
void Schrodinger1D::initializeLinearSolver()
{
	...
	M->setValue(m_Space->getCurrentGrid()->getSizeOfGrid()-1, m_Space->getCurrentGrid()->getSizeOfGrid()-2, 2.*alpha);
	M->setValue(m_Space->getCurrentGrid()->getSizeOfGrid()-1, m_Space->getCurrentGrid()->getSizeOfGrid()-1, 1.-2.*alpha);

	for(int i=1; (i<m_Space->getCurrentGrid()->getSizeOfGrid()-1); ++i)
	{
...
void Schrodinger1D::InitialState()
{
	...
	if(!LS)
		return;
	GridBase* D=m_Space->getCurrentGrid();
...
{
	//qDebug() << "call at";
	return m_Space->getCurrentGrid()->getValue(P);
}
